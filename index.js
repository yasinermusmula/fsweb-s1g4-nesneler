...
function MenuElemaniOlustur(name, cost, option) {
  /*Kodlar buraya*/
  serpmeKahvalti.isim = name;
  serpmeKahvalti.fiyat = cost;
  serpmeKahvalti.kategori = option;
  return serpmeKahvalti;
}
...
function DegerlendirmeEkle(dizi, isim, puan, geribildirim) {
  /*Kodlar buraya */
  const obj = { isim: isim, puan: puan, geribildirim: geribildirim };
  dizi.push(obj);
  return dizi;
}
console.log(
...
function SonDegerlendirmeyiAl(dizi) {
  /*Kodlar buraya*/
  return `${dizi[dizi.length - 2].isim} isimli kisi ${
    dizi[dizi.length - 2].puan
  } puan verdi ve sunları yazdi: ${dizi[dizi.length - 2].geribildirim}`;
}
...
function PuanaGoreDegerlendirmeAl(dizi, key) {
  /* Kodlar buraya */
  const newObj = [];
  for (let i = 0; i < dizi.length; i++) {
    if (dizi[i].puan >= key && dizi[i].puan < key + 1) {
      newObj.push(dizi[i]);
    }
  }
  return newObj;
...
function UzunDegerlendirmeleriAl(dizi) {
  ...
  const newObjArr = [];
  for (let i = 0; i < dizi.length; i++) {
    let kelimeSayisi = dizi[i].geribildirim.split(" ").length;
    if (kelimeSayisi > 15) {
      newObjArr.push(dizi[i]);
    }
  }
  return newObjArr;
}
...
function arabaYapici(arabaKm) {
	/* Kodlar buraya */
	const araba = {
	  km: arabaKm,
	  surus: function (funcKm) {
		this.km += funcKm;
	  },
	};
	return araba;
